...
export async function salvarConfiguracoesParoquia(dados: ConfigParoquia): Promise<void> {
  try {
    ...
    const d = dados || {};
    await db.execute(
      "UPDATE paroquia SET nome=?,diocese=?,cidade=?,estado=?,endereco=?,cep=?,email=?,telefone=?,cnpj=?,logo_path=?,diocese_logo_path=? WHERE id=(SELECT id FROM paroquia LIMIT 1)",
      [d.nome||"",d.diocese||"",d.cidade||"",d.estado||"",d.endereco||"",d.cep||"",d.email||"",d.telefone||"",d.cnpj||"",d.logo_path||"",d.diocese_logo_path||""]
    );
